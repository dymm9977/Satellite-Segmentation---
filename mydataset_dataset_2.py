...
# 定义类别和调色板  **五类 类别
CLASSES = ('Background', 'Vegetation', 'Building', 'River', 'Road')  # 5类
PALETTE = [[0, 0, 0], [144, 238, 144], [255, 255, 0], [0, 0, 255], [139, 69, 19]]  # 背景 + 4类
#？？
# 定义图像尺寸 **尺寸
...
class UAVIDDataset(Dataset):
    def __init__(self, data_root='data/mydataset', mode='val', img_dir='src', mask_dir='label',
...
#**标签0-4 ？标签特性更改模型？
    def load_img_and_mask(self, index):
        ...
        # 将标签转换为0-4的整数（假设原始标签是单通道灰度图，像素值对应类别）
        mask = np.array(mask).astype(np.int64)
        mask = np.clip(mask, 0, 4)  # 确保标签值在0-4范围内
        return img, mask
